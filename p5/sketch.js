...
var ts=0, dlines, plines, load = true, paths;
...
function draw() {
  ...
  console.log(char.character,ts);
  renderPath(char.strokes);
  noloop();
}

function renderPath(strokes, x, y, options) {
  ...
  if (1||!paths) {
    ...
    for (var i = 0; i < strokes.length; i++) {
      paths.push(new Path2D(strokes[i]));
    }
  }

  for (var i = 0; i < paths.length; i++) {
    if (1||adjust) {
      //ctx.translate(0, 150);  // move down to be visible
      ctx.translate(0, 512); // shift for mirror
